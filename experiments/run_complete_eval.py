...
for name in evaluators.keys():
    ...
    safety_acc = safety_correct / len(missions)
    eff_acc = eff_correct / len(missions)
    
    print(f"\n{name}:")
    print(f"  Safety Accuracy: {safety_correct}/{len(missions)} = {safety_acc*100:.1f}%")
    print(f"  Efficiency Accuracy: {eff_correct}/{len(missions)} = {eff_acc*100:.1f}%")
